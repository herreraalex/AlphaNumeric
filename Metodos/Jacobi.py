...
def Jacobi(A, b, t, iter, x0):
    ...
    l = len(A[0])
    if (n!=l):
        print("A no es una matriz cuadrada porfavor revise y ejecute nuevamente.")
    else:
        x =[None] * n
        aux=0
        cont = 0
        ...
        while(error > t and cont <= iter):
            print("itereración: #" + str(iteration))
            error = 0
            for i in range(0,n):
                suma = 0
                for j in range(0,n):
        ...
        if (error < t):
            ...
            print("no se llegó a la solución en " + str(iter) + " iteraciones")
            return        
